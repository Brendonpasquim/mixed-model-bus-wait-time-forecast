...
title("Tempos de Espera entre Pontos de Parada (Todos)", xlab="Sequência de Pontos de Parada", ylab="Tempo de Espera (Segundos)")
```
...
plot(1,
     ...
     xlab = "Sequência de Pontos de Parada")
...
# Saves graph as JPEG
ggsave(file.path(image_folder, "graph_m1_vs_m2_forecast_comparison.jpeg"), plot = graph_m1_vs_m2_forecast_comparison, width = 8, height = 6, dpi = 300)
```
...
ggplot(m2_forecasts_with_confidence_interval_whole_day, aes(x = seq_along(predicoes))) +
  ...
  labs(title = "Previsões com Intervalos de Confiança", x = "Sequência de Pontos de Parada", y = "Tempo de Espera (Segundos)") +
  theme_minimal()
```
...
graph_m2_forecast_1_cycle <- ggplot(dados1, aes(x = Seq)) +
      ...
      geom_line(aes(y = Limite_Inferior), color = "orange", linetype = "dashed") +  # Limite inferior
      geom_line(aes(y = Limite_Superior), color = "orange", linetype = "dashed") +  # Limite superior
      labs(title = "Previsões com Intervalos de Confiança (M2 - 1 Itinerário)", x = "Sequência de Pontos de Parada", y = "Tempo de Espera (Segundos)") +
      theme_minimal()
...
graph_m2_residuals_acf <- ggplot(m2_acf_df, aes(x = Lag, y = ACF)) +
  ...
  labs(
    title = "Gráfico de Autocorrelação (ACF) de M2 com IdC de M3",
    x = "Defasagem (Lag)",
    y = "Autocorrelação"
  ) +
  theme_minimal()

ggsave(file.path(image_folder, "graph_m2_residuals_acf_with_m3_confidence_interval.jpeg"), plot = graph_m2_residuals_acf, width = 8, height = 6, dpi = 300)
...
acf_plot1 <- ggplot(m3_acf_df, aes(x = Lag, y = ACF)) +
  ...
  labs(
    title = "Autocorrelação dos Resíduos Ajustados (ACF) de M3",
    x = "Defasagem (Lag)",
    y = "Autocorrelação"
  ) + theme_minimal()

ggsave(file.path(image_folder, "graph_m3_acf_adjusted_residuals.jpeg"), plot = acf_plot1, width = 8, height = 6, dpi = 300)
...
graph_m3_forecast_1_cycle <- ggplot(dados1, aes(x = Seq)) +
      ...
      labs(title = "Previsões com Intervalos de Confiança (M3 - 1 Itinerário)", x = "Sequência de Pontos de Parada", y = "Tempo de Espera (Segundos)") + theme_minimal()
...
graph_m3_with_sazonality_forecast_1_cycle <- ggplot(dados1, aes(x = Seq)) +
      ...
      geom_line(aes(y = Limite_Inferior), color = "orange", linetype = "dashed") +  # Lower Limit
      geom_line(aes(y = Limite_Superior), color = "orange", linetype = "dashed") +  # Higher Limit
      labs(title = "Previsões com Intervalos de Confiança (M3 Sazonal - 1 Itinerário)", x = "Sequência de Pontos de Parada", y = "Tempo de Espera (Segundos)") +
      theme_minimal()
...
acf_plot2 <- ggplot(acf_df, aes(x = Lag, y = ACF)) +
  ...
  labs(
    title = "Autocorrelação dos Resíduos Ajustados (ACF) (M3 - Sem outliers)",
    x = "Defasagem (Lag)",
    y = "Autocorrelação"
  ) +
  theme_minimal()

ggsave(file.path(image_folder, "graph_m3_with_sazonality_without_outliers.jpeg"), plot = acf_plot2, width = 8, height = 6, dpi = 300)
```